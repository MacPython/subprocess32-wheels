##########################################
Building and uploading subprocess32 wheels
##########################################
...
https://travis-ci.org/MacPython/subprocess32-wheels
...
* builds a subprocess32 wheel, linking against these fresh builds;
...
* making a commit to the `subprocess32-wheels` repository (e.g. with `git
...
Which subprocess32 commit does the repository build?
====================================================

The `subprocess32-wheels` repository will build the commit specified in the
...
be something like::
    ...
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t macosx subprocess32 $VERSION
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t manylinux1 subprocess32 $VERSION
...
``3.2.7`` is the version to download / upload.
...
``subprocess32-3.2.7-`` from http://wheels.scipy.org to ``~/wheelhouse``,
